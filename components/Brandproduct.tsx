...
const BrandProducts = ({ brands, slug }: BrandProductsProps) => {
  ...
  const getBrandTitle = (slug: string) => {
    return brands.find(b => b.slug?.current === slug)?.title || 'Marque inconnue';
  };
                ...
                <div className="p-2">
                  ...
                  {selectedBrands.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {selectedBrands.map(slug => (
                        ...
                        >
                          <span>{getBrandTitle(slug)}</span>
                          <button 
                            onClick={() => handleBrandSelection(slug)}
                  ...
                  <div className="h-[calc(100vh-250px)] overflow-y-auto">
                    {filteredBrands.map((brand) => (
                      <label key={brand._id} className="flex items-center space-x-2 py-1.5 hover:bg-gray-50 p-1 rounded">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className="text-sm text-gray-800">{brand.title}</span>
                      </label>
                    ))}
                  </div>
                </div>
